...
def draw_labels_and_scores(image, bbox, label, score):
    """在图像上绘制标签和置信分数。
    :param image: 原始图像
    :param bbox: 边界框
    :param label: 标签
    ...
    cv2.putText(image, text, (bbox[0], bbox[1] - 5), cv2.FONT_HERSHEY_SIMPLEX,
                0.5, (255, 0, 0), 2)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default='person, motor vehicle, bicycle, car, Tactile paving'
    )
...
def inference_detector(runner,
    ...
    for bbox, mask, label, score in zip(pred_instances['bboxes'],pred_instances['masks'], pred_instances['labels'], pred_instances['scores']):
        color = np.random.randint(0, 256, (3,)).tolist()  # 为每个mask生成随机颜色
        apply_mask(image, mask, color)
        # 这里需要有bbox来定位标签和分数，假设从mask中提取bbox
        ys, xs = np.where(mask == 1)
        if ys.shape[0] and xs.shape[0]:
            draw_labels_and_scores(image, bbox, texts[label], score)
